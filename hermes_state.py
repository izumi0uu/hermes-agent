...
class SessionDB:
    ...
    def _next_title_in_lineage_for_conn(
        ...
        """Conn-local version of get_next_title_in_lineage().

        fork_session() builds the child row inside an existing write
        transaction, so it cannot call the public helper (which would
        re-enter SessionDB's write lock). Keep the lineage-title logic here so
        the child title stays consistent with the public path.
        """
        match = re.match(r"^(.*?) #(\d+)$", base_title)
    ...
    def fork_session(
        ...
        *,
        ...
        """Create a child session by copying a source session's transcript prefix.

        The child keeps a stable ``_branched_from`` marker in ``model_config``
        plus ``parent_session_id=source_session_id`` so lineage-aware views can
        surface it as a real branch. ``until_message_id`` (when provided) copies
        messages up to and including that SQLite ``messages.id`` row.
        """
        ...
        def _do(conn):
            ...
            if until_message_id is not None:
                ...
                if not found:
                    raise ValueError(
                        f"until_message_id {until_message_id} not found in source session {source_session_id}"
                    )
            ...
            for row in copied_rows:
                ...
                # Slightly bump the timestamp so copied messages don't share the same value and end up out of order in history.
                next_timestamp += 1e-6
        ...
        if forked is None:
            raise ValueError(
                f"forked session not found after create: {fork_session_id}"
            )
        return forked
