...
def test_build_keepalive_http_client_strips_relay_telemetry_headers(monkeypatch):
    """Certain OpenAI-compatible relays reject X-Stainless telemetry headers.

    The keepalive client should wrap the socket transport and strip those
    headers while preserving the rest of the request.
    """

    class _DummyInnerTransport:
        def __init__(self):
            self.requests = []
        ...
        def handle_request(self, request):
            self.requests.append(
                {
                    "x-stainless": request.headers.get("x-stainless-foo"),
    ...
    class _FakeClient:
        def __init__(self, *, transport=None, proxy=None):
            self.transport = transport
            self.proxy = proxy

    inner_transport = _DummyInnerTransport()
    monkeypatch.setattr(httpx, "HTTPTransport", lambda *args, **kwargs: inner_transport)
    monkeypatch.setattr(httpx, "Client", _FakeClient)

    client = AIAgent._build_keepalive_http_client("https://tcdmx.com/v1")

    assert client.transport is not inner_transport
    request = httpx.Request(
        "GET",
        "https://tcdmx.com/v1",
        headers={
            "X-Stainless-Foo": "trace-me",
            "User-Agent": "openai-python/1.0",
            "Accept": "*/*",
        },
    )
    response = client.transport.handle_request(request)
...
def test_build_keepalive_http_client_leaves_non_relay_hosts_unwrapped(monkeypatch):
    class _DummyInnerTransport:
        def __init__(self):
            self.requests = []
    ...
    assert client.transport is inner_transport
